...
class FileNoteDataSource(
    ...
    fun setNoteDate(file: File, timestamp: Long): Boolean {
        return try {
            file.setLastModified(timestamp)
            true
        } catch (e: Exception) {
            Log.e("FileNoteDataSource", "Ошибка установки даты файла: ${e.message}")
            false
        }
    }

    // Получение текущей даты файла
    fun getNoteDate(file: File): Long {
